...
static void vlist_insert(vlist_head_t *new, vlist_head_t *prev,
			vlist_head_t *next)
{
	new->prev = prev;
	new->next = next;
...
{ vlist_insert(new, head, head->next); }

void vlist_add_tail(vlist_head_t *new, vlist_head_t *head)
{ vlist_insert(new, head->prev, head); }

static void __vlist_del(vlist_head_t *prev, vlist_head_t *next)
{
	prev->next = next;
	next->prev = prev;
}
...
{ __vlist_del(entry->prev, entry->next); }

static void __vlist_splice(vlist_head_t *vlist, vlist_head_t *prev,
			vlist_head_t *next)
{
	vlist_head_t *first = vlist->next;
	vlist_head_t *last = vlist->prev;
...
{
	if (!vlist_empty(vlist)) {
		__vlist_splice(vlist, head, head->next);
		vlist_init(vlist);
	}
}
...
size_t vlist_size(const vlist_head_t *head)
{
	...
	for (const vlist_head_t *pos = head->next; pos != head;
		pos = pos->next)
		++size;
	return size;
}
